...
bool isCorrupt(message* mes) {
	...
	cout << calChecksum((uint16_t*)mes) << endl;
	return true;
}
...
//缓存窗口内包
void message::storeMes(message* Mes) {
	this->messeq = Mes->messeq;
	this->filelen = Mes->filelen;
	this->FLAGS = Mes->FLAGS;
	memcpy(this->data, Mes->data, Mes->filelen);
	return;
}