		...
		case 0:	//等待数据包头文件状态
			...
			if (res > 0) {
				if (randomNum(randomGen) <= LOSS_RATE) {//主动丢包
					...
					else if (isCorrupt(recvMes) || has_seq0(recvMes)) {
						cout << "数据包接收失败，请求重传" << endl;
						//发送ACKseq=0的包
						sendACK(0);
					}
					else if (!isCorrupt(recvMes) && has_seq1(recvMes)) {
		...
		case 1:
			...
			if (res > 0) {
				if (randomNum(randomGen) <= LOSS_RATE) {//主动丢包
					...
					printTime();
					printRecvMessage(recvMes);
					if (isCorrupt(recvMes) || has_seq1(recvMes)) {
						//发送ACKseq=1的包，申请重发
						cout << "数据包损坏或顺序有误，申请重发！" << endl;
						sendACK(1);
					}
					else if (!isCorrupt(recvMes) && has_seq0(recvMes)) {
						//收到了目标的包
		...
		case 2:
			...
			if (res > 0) {
				if (randomNum(randomGen) <= LOSS_RATE) {//主动丢包
					...
					printTime();
					printRecvMessage(recvMes);
					if (isCorrupt(recvMes) || has_seq0(recvMes)) {
						//发送ACKseq=0的包，申请重发
						cout << "数据包损坏或顺序有误，申请重发！" << endl;
						sendACK(0);
					}
